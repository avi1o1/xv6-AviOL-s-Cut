...
int main(int argc, char *argv[]) {
    ...
    // Send initial message to server to indicate readiness
    const char* init_msg = "ready";
    sendto(sock, init_msg, strlen(init_msg), 0, (struct sockaddr*)&serv_addr, addr_len);
    ...
    while (1) {
        ...
        if (strncmp(buffer, "myTurn", 6) == 0) {
            ...
            usleep(100000);
            sendto(sock, input, strlen(input), 0, (struct sockaddr *)&serv_addr, addr_len);
        }
        ...
        // Someone disconnected
            ...
            if (strncmp(input, "exit", 4) == 0) {
                ...
                printf("\033[1;31m Oopsie Woopsie : The other player disconnected!\033[0m\n");
            }
            break;
        }
