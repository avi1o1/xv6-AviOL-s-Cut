...
struct proc
{
  ...
  int alarm_on;                // Flag to indicate if alarm is set
  int handling_alarm;          // Flag to indicate if we're handling an alarm
  
  #ifdef LBS
  int tickets;                 // Number of lottery tickets
  uint arrival_time;           // Arrival time of the process
  #endif

  #ifdef MLFQ
  int priority;                // Current priority level (0-3)
  int ticks;                   // Ticks used in current priority level
  int time_slice;              // Remaining time slice for the current priority
  #endif
};
